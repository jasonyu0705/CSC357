...
int main(int argc, char *argv[]){
   ...
   //bit position poinrters
    ...
    //writing the file
    FILE* outptr=fopen(OutputFile.c_str(),"wb");
    //writing the file header
    tagCOMPRESSFILEHEADER CFH;
    tagCOMPRESSINFOHEADER CIH;

    //getting the number of bytes for each colour
    ...
    CIH.redSize = bitPosRed / 8 + (bitPosRed % 8 != 0);  // Ensure byte alignment
    CIH.greenSize = bitPosGreen / 8 + (bitPosGreen % 8 != 0);
    CIH.blueSize = bitPosBlue / 8 + (bitPosBlue % 8 != 0);
